...
st.markdown("""
    ...
    /* Títulos con azul oscuro estilo ejecutivo */
    .main-title { font-size: 28px; font-weight: bold; color: #003366; margin-bottom: 5px; border-bottom: 2px solid #003366; padding-bottom: 5px;}
    .subtitle { font-size: 14px; color: #333333; margin-bottom: 25px; font-weight: 500;}
    
    /* Panel superior de filtros (estilo cabecera clara) */
    .sticky-filters {
        position: sticky;
        top: 0px;
        z-index: 999;
        background-color: #DCE6F1; /* Azul claro estilo interfaz bancaria */
        padding: 15px 15px;
    ...
    /* Forzar contraste en los textos nativos de Streamlit */
    h1, h2, h3, h4, p, span, label {
        color: #003366 !important;
    }
    
    /* Pestañas estilo ejecutivo */
    .stTabs [data-baseweb="tab"] { color: #003366; font-weight: bold; }
    </style>
...
def crear_reloj(valor, titulo, objetivo, max_val, color_ok="#2ecc71", color_bad="#e74c3c"):
    ...
    fig = go.Figure(go.Indicator(
        ...
        number={'suffix': "%" if "NPS" in titulo else "", 'font': {'size': 40, 'color': '#003366'}},
        ...
        gauge={'axis': {'range': [-100 if "NPS" in titulo else 0, max_val], 'tickwidth': 1, 'tickcolor': '#003366'}, 
               ...
               'borderwidth': 1,
               'bordercolor': "#003366",
               'steps': [{'range': [-100 if "NPS" in titulo else 0, objetivo], 'color': 'rgba(231, 76, 60, 0.15)'}, 
                         {'range': [objetivo, max_val], 'color': 'rgba(46, 204, 113, 0.15)'}],
               'threshold': {'line': {'color': "#003366", 'width': 3}, 'thickness': 0.75, 'value': objetivo}}
    ))
    # Fondo blanco sólido para el componente del reloj
    fig.update_layout(height=280, margin=dict(l=10, r=10, t=40, b=10), paper_bgcolor="white", font={'color': "#003366"})
    return fig
...
if not df_ventas_raw.empty:
    ...
    with tab_convencional:
        ...
        if not df_tabla_mensual.empty:
            ...
            fig_evolucion.add_trace(go.Bar(
                ...
                name='Cant. Encuestas', marker_color='#9EABC0', 
            ...
            fig_evolucion.add_trace(go.Scatter(
                ...
                mode='lines+markers+text', name='SSI Puro', line=dict(color='#004B87', width=3), 
            ...
            fig_evolucion.update_layout(
                title=dict(text="Evolución de SSI, NPS y Volumen de Encuestas", font=dict(color='#003366', size=20)),
                yaxis=dict(title="Puntaje / Porcentaje", range=[-100, 110], zeroline=True, zerolinecolor='rgba(231, 76, 60, 0.5)', zerolinewidth=2, tickfont=dict(color='#003366'), titlefont=dict(color='#003366'), showgrid=True, gridcolor='#E0E0E0'),
                ...
                xaxis=dict(tickfont=dict(color='#003366')),
            ...
            fig_pie_0km.update_layout(paper_bgcolor="white", plot_bgcolor="white", font=dict(color='#003366'), title_font=dict(size=18))
            ...
            fig_bar_suc_0km.update_layout(paper_bgcolor="white", plot_bgcolor="white", font=dict(color='#003366'), xaxis_title="Sucursal", yaxis_title="Cantidad", title_font=dict(size=18))
    ...
    with tab_ranking:
        ...
        if not df_resumen.empty:
            ...
            fig_ranking.update_layout(
                ...
                xaxis=dict(title="Volumen de Encuestas", range=[0, x_max_rank], showgrid=True, gridcolor='#E0E0E0', tickfont=dict(color='#003366'), titlefont=dict(color='#003366')), 
            ...
            if top_5_cols:
                ...
                if res_u:
                    ...
                    fig_evo_u.add_trace(go.Scatter(
                        ...
                        mode='lines+markers+text', name='SSI UCT', line=dict(color='#004B87', width=3), 
                    ...
                    fig_evo_u.update_layout(
                        title=dict(text="Evolución de SSI, NPS y Volumen de Encuestas (UCT)", font=dict(color='#003366', size=20)),
                        yaxis=dict(title="Puntaje / Porcentaje", range=[-100, 110], zeroline=True, zerolinecolor='rgba(231, 76, 60, 0.5)', zerolinewidth=2, tickfont=dict(color='#003366'), titlefont=dict(color='#003366'), showgrid=True, gridcolor='#E0E0E0'),
                    ...
                    fig_pie_uct.update_layout(paper_bgcolor="white", plot_bgcolor="white", font=dict(color='#003366'), title_font=dict(size=18))
    ...
    with tab_tpa:
        ...
        if df_t_proc is not None:
            ...
            with ct2:
                st.markdown(f'''
                    <div style="background-color:#FFFFFF; padding:15px; border-radius:8px; border-left:5px solid #E3000F; box-shadow:0 2px 5px rgba(0,0,0,0.05); text-align:center; height:100%; display:flex; flex-direction:column; justify-content:center;">
                        <span style="color:#555; font-size:16px; font-weight:bold;">TOTAL DE ENCUESTAS VÁLIDAS (TPA)</span><br>
                        <span style="font-size:48px; font-weight:bold; color:#2C3E50;">{len(df_nps_valid_t)}</span>
                    </div>
                ''', unsafe_allow_html=True)
                ...
                for idx, suc in enumerate(sucursales_validas):
                    ...
                    with barras_tpa_cols[idx]:
                        ...
                        fig_bar_tpa.update_layout(
                            ...
                            xaxis=dict(showgrid=True, gridcolor='#E0E0E0', tickfont=dict(color='#003366')),
                            yaxis=dict(tickfont=dict(color='#003366'), titlefont=dict(color='#003366'))
                        )
            ...
            with c2:
                st.markdown(f'''
                    <div style="background-color:#FFFFFF; padding:15px; border-radius:8px; border-left:5px solid #E3000F; box-shadow:0 2px 5px rgba(0,0,0,0.05); text-align:center; height:100%; display:flex; flex-direction:column; justify-content:center;">
                        ...
                        <span style="font-size:48px; font-weight:bold; color:#2C3E50;">{len(df_nps_valid_t26)}</span>
                    </div>
                ''', unsafe_allow_html=True)
            ...
            if col_proyecto and col_pdv and col_resultado:
                ...
                if not df_myst_filt.empty:
                    ...
                    fig_myst.update_layout(
                        ...
                        yaxis=dict(range=[0, max(100, df_myst_agrupado['Resultado_Num'].max() * 1.2)], zeroline=False, tickfont=dict(color='#003366'), titlefont=dict(color='#003366')),
